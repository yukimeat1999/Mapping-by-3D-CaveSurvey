...
{
  ContourTestInit(manager);
  ContourInit(manager);
  RTC::RtcBase* comp;

  // Create a component
  comp = manager->createComponent("ContourTest");
...
bool RunTest()
{
  ...
  comp = mamager.getComponent("ContourTest0");
  if (comp == nullptr)
  {
  ...
  ContourTest* testcomp = dynamic_cast<ContourTest*>(comp);
  if (testcomp == nullptr)
  {
    std::cerr << "Component get failed." << std::endl;
    return false;
