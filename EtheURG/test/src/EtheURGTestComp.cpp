...
{
  EtheURGTestInit(manager);
  EtheURGInit(manager);
  RTC::RtcBase* comp;

  // Create a component
  comp = manager->createComponent("EtheURGTest");
...
bool RunTest()
{
  ...
  comp = mamager.getComponent("EtheURGTest0");
  if (comp == nullptr)
  {
  ...
  EtheURGTest* testcomp = dynamic_cast<EtheURGTest*>(comp);
  if (testcomp == nullptr)
  {
    std::cerr << "Component get failed." << std::endl;
    return false;
