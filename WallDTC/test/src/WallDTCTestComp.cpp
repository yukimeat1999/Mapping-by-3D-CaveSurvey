...
{
  WallDTCTestInit(manager);
  WallDTCInit(manager);
  RTC::RtcBase* comp;

  // Create a component
  comp = manager->createComponent("WallDTCTest");
...
bool RunTest()
{
  ...
  comp = mamager.getComponent("WallDTCTest0");
  if (comp == nullptr)
  {
  ...
  WallDTCTest* testcomp = dynamic_cast<WallDTCTest*>(comp);
  if (testcomp == nullptr)
  {
    std::cerr << "Component get failed." << std::endl;
    return false;
